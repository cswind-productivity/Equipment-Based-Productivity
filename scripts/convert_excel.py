...
- GitHub_Export 시트를 우선 읽습니다.
- GitHub_Export가 수식 링크라서 GitHub Actions에서 계산값이 비어 있으면 Raw(1)로 자동 fallback합니다.
- Weekly + Monthly rawData를 dashboard_template.html에 삽입하여 index.html을 생성합니다.
"""
import pandas as pd
import json
import sys
...
def clean_df(df):
    ...
    df = df[REQUIRED + [c for c in ['Month', 'YearMonth', 'Month_Sort'] if c in df.columns]]
...
def read_source(excel_path):
    # 1) GitHub_Export 우선
    try:
        df_export = pd.read_excel(excel_path, sheet_name='GitHub_Export', engine='openpyxl')
        df_export = clean_df(df_export)
        if len(df_export) > 0:
            print(f"✅ GitHub_Export 시트 사용: {len(df_export)} rows")
            return add_month_columns(df_export)
    except Exception as e:
        print(f"⚠️ GitHub_Export 시트 사용 불가 또는 계산값 없음: {e}")

    # 2) fallback: Raw(1)
    print("↪ Raw(1) 시트로 fallback합니다.")
    df_raw = pd.read_excel(excel_path, sheet_name='Raw(1)', engine='openpyxl')
    df_raw = clean_df(df_raw)
    print(f"✅ Raw(1) 시트 사용: {len(df_raw)} rows")
    return add_month_columns(df_raw)

def get_val(df_fac, type_, cat):
    v = df_fac[(df_fac['Type'] == type_) & (df_fac['Category'] == cat)]["Q'ty"].sum()
    return float(v) if not pd.isna(v) else 0.0

def convert_by_period(df, period_col):
    raw_data = {}
...
def build_index_html(weekly_raw_data, monthly_raw_data, template_path, output_path):
    ...
    if '%%RAWDATA_PLACEHOLDER%%' not in template:
        print("❌ dashboard_template.html에 %%RAWDATA_PLACEHOLDER%%가 없습니다.")
        sys.exit(1)
...
def main():
    ...
    print("완료: Weekly + Monthly 데이터가 index.html에 반영되었습니다.")
